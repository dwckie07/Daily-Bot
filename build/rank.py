...
# Hàng đợi giới hạn tối đa 1 tác vụ vẽ PIL tại một thời điểm (Tránh tràn RAM Render)
DRAW_SEMAPHORE = asyncio.Semaphore(1)
...
def draw_leaderboard_sync(page_data, page_avatars, author_id, author_rank_idx, author_info, author_avatar, guild_member_names, current_page, total_pages):
    card_height, card_gap = 76, 10
    top_margin, bottom_margin = 150, 25
    width = 950
        ...
        try:
            ...
            if data:
                ...
                for idx, (uid, info) in enumerate(sorted_users, start=1):
                    if uid == user_id:
                        rank_str = f"🥇 Top 1" if idx == 1 else (f"🥈 Top 2" if idx == 2 else (f"🥉 Top 3" if idx == 3 else f"#{idx}"))
                        break
